...
class RekapChecklist extends Page implements HasForms
{
    ...
    {
        // dd("test");
        return 'filament.resources.checklists.pages.rekap-checklist';
    }
    ...
    protected function loadRekap(): void
    {
        ...
        $checklists = Checklist::where('instansi_id', $this->instansi_id)
        ...
        foreach ($members as $member) {
            $memberChecklist = $checklists->get($member->id, collect());
            $checklistByDate = $memberChecklist->keyBy(fn ($a) => Carbon::parse($a->created_at)->format('Y-m-d'));
            ...
            foreach ($this->tanggalList as $tgl) {
                $checklist = $checklistByDate->get($tgl['full_date']);
                
                $row['checklist'][$tgl['tanggal']] = [
                    'isAny' => !!$checklist,
    ...
    public function exportPdf()
    {
        ...
        $filename = 'rekap-checklist-' . str()->slug($this->periode['instansi'] ?? 'all') . '-' . $this->bulan . '-' . $this->tahun . '.pdf';

        // Increase memory limit temporarily
        ini_set('memory_limit', '256M');

        $pdf = Pdf::loadView('exports.rekap-checklist-instansi-pdf', [
            'rekapData' => $pdfData,
